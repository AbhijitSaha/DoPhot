	...
	COMMON /FITARRAYS/ A(NPMAX), FA(NPMAX), C(NPMAX+1,NPMAX+1),
     1    B(2*NPMAX), FB(2*NPMAX)
        common /skyvar/ skypar(npsky)
        common /perfs/ pkprfmn
...
2758 	IF (I .LE. NSTOT) THEN
	  ...
	  PERFECT = GOODSTAR .AND. type1 .and. (STARPAR(2,I) .gt.
     $      pkprfmn)
	  ...
	  IF (NPERF .GE. MINRMS) THEN
	    DO 2763 I = 1, NSTOT
	      PERFECT = IMTYPE(I) .EQ. 1 .or. imtype(i) .eq. 11		
              PERFECT = PERFECT .and. (STARPAR(2,I) .gt. pkprfmn)
