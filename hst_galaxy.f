...
c.... begin crash prevention check and CHI(2) calculation:
          CHI(2) = 2.*alog10(amax1(abs(A(6)-B(6)), .001)) - 
     $       alog10( abs(TOT(2) + ERR(6)) )
          if( CHI(2) .le. 5. ) then 
...
c... Make Absolute value check for HST data in addition to CHICRIT comparison:
	  GALAXY = (CHI(4) .GE. CHICRIT) .or. 
     $      (A(5) .gt. B(5)*2.) .or. (A(7) .gt. B(7)*2.)
c... End check
	END IF
	RETURN
	END 
