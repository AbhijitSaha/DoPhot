...
c
	...
	common / / ibig(ncmax*nrmax/2), isig(ncmax*nrmax/2)
        common /median/ MEDPIX 
        common /trans7/ test7
        common /fixpass/ fixxy
c
	LOGICAL needit, first, warm, test7
c
	character*80 header(100), sigfile
...
c     *      call readlocalf(nhead,header,nx,ny,ibig,files(1))
      ...
      nslow = ny
c
c  -- read in sigma file:
        write(*,*) ' Enter name of sigma file (I*2) : ' 
        read(*, 999) sigfile
        call rffile(nhead, header, nxs, nys, isig, sigfile)
        if( (nx .ne. nxs) .or. (ny .ne. nys) ) then
          write(*,*) ' Sizes of image and noise-image not the same !!'
          write(*,*) nx, ny, nfast, nslow
          write(*,*) ' Stopping'
          stop
        end if 
c
...
c
	...
	CALL MAKENOISE(ibig,noise,NFAST,NSLOW,isig)
c
c...  MAKENOISE returns actual values of sigma used in isig... SO:
        call wffile(nhead, header, nxs, nys, isig, 'sigma_used.fit')
