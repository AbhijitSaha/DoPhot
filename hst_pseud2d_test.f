C:  UNTESTED
c:  Finite binning for TWOFIT is turned off by setting corr=0 for PSEUD4D
	...
	DIMENSION A(10), FA(7)
	real*8 PP(2)
        real*8 DENOM, DDDT, DBLDT, P1, Q1, PEXP, CORR
        real*8 A2, A5, A7, A8, A10, T5, T6, T7, T8, T9, T10, U8, U9          
	INTEGER*2 IX(2)
	LOGICAL NEEDIT
	DATA HALF, THIRD, SIXTH / 0.50000000, 0.3333333, 0.16666666/
	DATA EXPMIN / -23. /
	DATA FMINCORR, FMAXCORR / -0.5, 1.0 /
	DATA FMINCORR4, FMAXCORR4 / 0, 0.0 /
...
c	DATA beta8 / 2.5 /
	...
	IF (NEEDIT) THEN
	  ...
	  if (corr .le. FMINCORR) then
	    ...
	    fc6 = 0.
	    fc7 = 0.
c	   fc3 = -2.*(a5*t8 + a(6)*t9)/24.
c	   fc4 = -2.*(a(6)*t8 + a7*t9)/24.
c	   fc5 = -(2*x*t8 - 1)/24./a(5)**2
c	   fc7 = -(2*y*t9 - 1)/24./a(7)**2
c	   fc6 = 2*(y*t8 + x*t9)/24.
	  end if
	...
	DO 2757 I = 1, 2
     ...
     +            (beta8/6.)*T2*T2
	  ...
	  if (corr .le. FMINCORR4) then
	   corr = FMINCORR4
	   fc3 = 0.
	   fc4 = 0.
	  else if (corr .ge. FMAXCORR4) then
	   corr = FMAXCORR4
	   fc3 = 0.
	   fc4 = 0.
	  else
