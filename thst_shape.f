...
c
	...
	LOGICAL CONVERGE, test7, OBLIT
        LOGICAL COSM, COSPROB, HOLE
...
c	      CONVERGE = GALCHI .LT. 1E10
              CONVERGE = GALCHI .LT. 1E11
	      OFFP = OFFPIC(A,IX,IY,NFAST,NSLOW,DX,DY)
c...... Check on objects that GALAXY skips over and can keep as type 1
              IF(A(5) .GT. 2.*gxwid .OR. A(7) .GT. 2.*gywid .or.
     *          abs(A(6)) .gt. 5.0*amax1(1./gxwid, 1./gywid)  ) then
                  CONVERGE = .FALSE.
                  if(lverb .gt. 20) then
                    write(6,*) ' (SHAPE...) Object', I, ' at', IX, IY,
     *     '  has undeterminable shape '
                    write(6,*) gxwid, gywid, A(5), A(6), A(7),
     *        SHADERR(5,I), SHADERR(6,I), SHADERR(7,I)
                  end if
              END IF
              COSM = COSPROB(A,SHADERR(1,I), STARPAR(1,I))
              if(COSM .and. JMTYPE .ne. 1) then 
                 starpar(2,I) = -32768
                 starpar(5,i) = widobl
                 starpar(6,i) = -1.
                 starpar(7,i) = widobl
         hole = oblit(onestar, big, noise, nfast, nslow, starpar(1,i))
              JMTYPE = 8 
                  if(lverb .gt. 20) then
                    write(6,*) ' (SHAPE...) Object', I, ' at', IX, IY,
     *  ' has been set as a Cosmic Ray'
                  end if
              go to 1234
              end if
c...... END Check
